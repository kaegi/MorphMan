...
def getPath( le, caption, open_directory=False ): # LineEdit -> GUI ()
    ...
    try:
        if open_directory:
            path = QFileDialog.getExistingDirectory( caption=caption, directory=start_path, options=QFileDialog.ShowDirsOnly )
        else:
...
class MorphMan(QDialog):
    def __init__(self, parent=None):
        ...
        self.ui.morphemizerLabel.setText('Dictionary: %s %s' % (self.morphemizer.getDescription(), self.morphemizer.getDictionary()))
    ...
    def onAnalyze( self ):
        ...
        if os.path.isfile(master_freq_path):
            with io.open(master_freq_path, 'r', encoding='utf-8-sig') as csvfile:
        ...
        def accepted_filetype(filename):
            ...
            if lower_filename.endswith('.srt'):
                return True
            if lower_filename.endswith('.ass'):
                return True
        ...
        if len(list_of_files) > 0:
            ...
            for n, file_path in enumerate(sorted(list_of_files, key=natural_keys)):
                mw.progress.update( value=n, label='Parsing (%d/%d) %s' % (n+1, len(list_of_files), os.path.basename(file_path)) )
